...
function M.load(theme)
    local utils = require("kanagruvbox.utils")
    ...
    if M.config.compile then
        ...
        local colors = require("kanagruvbox.colors").setup({ theme = theme, colors = M.config.colors })
        local highlights = require("kanagruvbox.highlights").setup(colors, M.config)
        require("kanagruvbox.highlights").highlight(highlights, M.config.terminalColors and colors.theme.term or {})
    end
end

function M.compile()
    for theme, _ in pairs(require("kanagruvbox.themes")) do
        local colors = require("kanagruvbox.colors").setup({ theme = theme, colors = M.config.colors })
        local highlights = require("kanagruvbox.highlights").setup(colors, M.config)
        require("kanagruvbox.utils").compile(theme, highlights, M.config.terminalColors and colors.theme.term or {})
    end
end

vim.api.nvim_create_user_command("KanagawaCompile", function()
    for mod, _ in pairs(package.loaded) do
        if mod:match("^kanagruvbox%.") then
