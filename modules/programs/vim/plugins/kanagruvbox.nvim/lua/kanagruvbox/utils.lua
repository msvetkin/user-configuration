...
local get_compiled_path = function(theme)
    return table.concat({vim.fn.stdpath("state"), "kanagruvbox", theme .. "_compiled.lua"}, PATH_SEP)
end

---@return string theme
function M.get_theme_from_bg_opt()
    local config = require("kanagruvbox").config
...
function M.compile(theme, highlights, termcolors)
    vim.loop.fs_mkdir(vim.fn.stdpath("state") .. PATH_SEP .. "kanagruvbox", 448)
    ...
    local lines = {
        "require'kanagruvbox'.compiled = string.dump(function()",
        "local g = vim.g",
    ...
    file:write(require("kanagruvbox").compiled)
    file:close()
end
