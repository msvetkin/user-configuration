...
function M.setup(opts)
    opts = opts or {}
    local override_colors = opts.colors or require("kanagruvbox").config.colors
    local theme = opts.theme or require("kanagruvbox")._CURRENT_THEME -- WARN: this fails if called before kanagruvbox.load()

    if not theme then
        error("kanagruvbox.colors.setup(): Unable to infer `theme`. Either specify a theme or call this function after ':colorscheme kanagruvbox'")
    end
    ...
    local theme_colors = require("kanagruvbox.themes")[theme](updated_palette_colors)
